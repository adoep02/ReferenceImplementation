...
public class StartUpAction extends CustomAction {
	...
	protected void initializeCodeFragments() {
		...
		addCodeFragment(new CodeFragment() {
			...
			@Override
			public void execute(final MD2Application app) {
				app.getEventBus().subscribe("saveButton_Touched", "saveButton_Touched_DataAction_GroupContentProvider_CreateOrUpdate;", new MD2EventHandler() {
					@Override
					public void eventOccured() {
		...
		addCodeFragment(new CodeFragment() {
			...
			@Override
			public void execute(final MD2Application app) {
				app.getEventBus().subscribe("saveButton2_Touched", "saveButton2_Touched_GotoViewAction_contactsTab", new MD2EventHandler() {
					@Override
					public void eventOccured() {
						if(app.getActiveActivity() instanceof TabbedActivity) {
		...
		addCodeFragment(new CodeFragment() {
			...
			@Override
			public void execute(final MD2Application app) {
				app.getEventBus().subscribe("saveButton_Touched", "saveButton_Touched_GotoViewAction_detailTab", new MD2EventHandler() {
					@Override
					public void eventOccured() {
						if(app.getActiveActivity() instanceof TabbedActivity) {
							((TabbedActivity)app.getActiveActivity()).setSelectedTab("detailTab");
